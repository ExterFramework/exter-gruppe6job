...
function getRandomStops(totalpickupNumber, typeC)
    local sourceList = typeC == 1 and Config.Pickups or Config.Atms
    local pickups = Utils.Functions.deepCopy(sourceList)
    ...
    for i = 1, numStops do
        ...
        if i == numStops then
            pickupsInRange = remainingPickups
        else
...
if framework == 'qbcore' or framework == 'qbox' then
    Core.Functions.CreateUseableItem('g6cashbag', function(source)
